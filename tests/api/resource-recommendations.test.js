...
import handler from "../../api/public/resource-recommendations.js";
...
test("POST /api/public/resource-recommendations returns tier-specific resources", async () => {
  const res = await invokeHandler(handler, {
    method: "POST",
    url: "/api/public/resource-recommendations",
    body: {
      tierId: "fundable_review",
      fundingPurpose: "working_capital",
  ...
  assert.equal(res.body.tierId, "fundable_review");
  assert.ok(Array.isArray(res.body.resources));
  assert.ok(res.body.resources.length >= 1);
  assert.ok(res.body.resources.every((resource) => resource.id && resource.title && resource.summary));
  assertPublicSafeObject(res.body);
});

test("POST /api/public/resource-recommendations adds nurture resources for not-ready tier", async () => {
  const res = await invokeHandler(handler, {
    method: "POST",
    url: "/api/public/resource-recommendations",
    body: {
      tierId: "not_ready_fixable",
  ...
  assert.equal(res.statusCode, 200);
  assert.equal(res.body.ok, true);
  assert.ok(res.body.resources.some((resource) => /readiness/i.test(resource.title)));
  assertPublicSafeObject(res.body);
});
